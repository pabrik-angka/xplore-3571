// js/table.js
/**
 * Xplore 3571 - Table Engine Controller
 * Bertanggung jawab mengendalikan Tampilan DaisyUI Table, Sticky Columns/Headers, Tab Navigasi, 
 * Debouncing Search, Multi-column Sorting, Pagination, dan Export CSV.
 */
import { Store } from './store.js';
import { DataFormatters } from './helpers/formatters.js';
import { openPivotModal } from './components/pivot-modal.js';
...
export const TableEngine = {
  ...
  init() {
    ...
    // Listener Event Reaktif State Store
    document.addEventListener('tabulation:changed', () => this.render());
  ...
  renderTableBody() {
    ...
    // Sync input pencarian global dengan filterState tab aktif
    ...
    // 1. Render Table Head (Header Sticky top-0)
    headEl.innerHTML = `
      <tr>
      ...
      th.addEventListener('click', () => {
        ...
        if (existingIdx >= 0) {
          if (activeTab.sortState[existingIdx].dir === 'asc') {
            ...
            activeTab.sortState.splice(existingIdx, 1); // Remove sort
          }
        } else {
    ...
    paginatedData.forEach((row, rowIdx) => {
      ...
      // Cek apakah row memiliki koordinat lat/lng untuk linked view
      ...
      if (hasCoords) {
        ...
        tr.addEventListener('click', () => {
          // Trigger Custom Event → ditangkap oleh MapEngine / Router
          window.location.hash = '#map';
          setTimeout(() => {
            document.dispatchEvent(new CustomEvent('explore:flyto', { detail: { lat, lng } }));
          }, 100);
        });
      }
